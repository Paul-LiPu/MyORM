...
import me.lipu.myorm.utils.JDBCUtils;
import me.lipu.myorm.utils.ReflectUtils;
...
public class MySqlQuery implements Query {
	...
	public Object queryByPriKey(Class clazz, Object id) {
		TableInfo tableInfo = TableContext.poClassTableMap.get(clazz);
		//获得主键
		ColumnInfo onlyPriKey = tableInfo.getOnlyPriKey();
		String sql = "select * from " + tableInfo.getTname() + " where "+onlyPriKey.getName() + "=? ";
		List list = queryRows(sql, clazz, new Object[]{id});
		return (list==null&&list.size()>0)?null:list.get(0);
	}
